...
import { useJoinTeamMutation } from "apollo/codegen";
...
const JoinTeam = () => {
  ...
  const [createTeam, { loading }] = useJoinTeamMutation({
    refetchQueries: ["Team", "User"]
  });
  const router = useRouter();

  const isDisabled = loading || !secret;

  const handleSubmit = async () => {
    try {
      await createTeam({ variables: { secret } });
      await router.push(ROUTES.APP_ROOT);
    } catch (e) {
      toast.error(
        "Unable to join team, are you sure you have the right secret?"
      );
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="space-y-2">
        <p className="text-sm">Please enter the team&apos;s invite token:</p>
        <TextInput onChange={e => setSecret(e.target.value)} />
        <button
          type="submit"
          className={clsx(
